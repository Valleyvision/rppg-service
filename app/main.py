...
Code Author : Krish
Code Ownership : ValleyVision 
Description : Remote PPG (rPPG) FastAPI service for webcam-based vital-sign estimation.

What it does
- Captures face ROI from a local camera and extracts green/red channel signals.
- Band-pass filters the signal and estimates heart rate via FFT and peak timing.
- Derives RR intervals and HRV features (SDNN, RMSSD, pNN50, LF, HF, LF/HF).
- Estimates SpO₂, perfusion index, and rough hemoglobin proxy from RGB ratios.
- Produces coarse systolic/diastolic BP via a simple HR-linked linear model.
- Classifies stress level from SDNN.
- Saves all readings plus robust aggregates to JSON.
- Optionally generates 5 baseline follow-up questions using a local LLM (Ollama).

Last update : 21.09.2025:20:59:00
'''
...
app = FastAPI(title="rPPG Robust Service", version="1.2.0")
...
# -----------------------------
# Signal helpers (unchanged except imports)
# -----------------------------
...
def capture_buffers(seconds: float) -> Tuple[np.ndarray, np.ndarray, float]:
    ...
    cap.release()
    if len(buf_g) < FS * 2: raise RuntimeError("Insufficient data captured")
...
# -----------------------------
# NEW: questions from metrics via local Llama (Ollama)
# -----------------------------
def _build_prompt(metrics: dict) -> str:
    ...
    lines = [
        ...
        "Do not diagnose. No advice. One question per line. Focus on context that helps interpret physiological metrics.",
        "",
        "Key aggregates:",
        ...
        "",
        "Output strict JSON array of 5 strings. No preface, no trailing text."
    ]
    return "\n".join(lines)
...
@app.post("/questions", response_model=LLMQuestionsOut, summary="Create 5 baseline questions from a saved metrics JSON using local Llama")
async def questions_from_metrics(body: LLMQuestionsIn):
    ...
    raw = await _ollama_generate(prompt, model=model, temperature=temp)
    ...
    try:
        ...
        # Fallback: split by newline and take first 5 non-empty lines
